...
export default function LiquidEther({
  ...
  const webglRef = useRef<WebGLManagerRef>(null);
  ...
  useEffect(() => {
    ...
    if (color0) {
      const c = new THREE.Color(color0);
      bgVec4.set(c.r, c.g, c.b, 1);
    }
    ...
    class MouseClass {
      ...
      timer: number | ReturnType<typeof setTimeout> | null = null;
      container: HTMLElement | null = null;
...
`;
    ...
    class ExternalForce extends ShaderPass {
      ...
      init(simProps: ExternalForceSimProps) {
        super.init(simProps);
      ...
      update(props: { cursor_size: number; mouse_force: number; cellScale: THREE.Vector2 }) {
        ...
        const uniforms = (this.mouse.material as THREE.RawShaderMaterial).uniforms as Record<string, { value: THREE.Vector2 }>;
    ...
    class Viscous extends ShaderPass {
      constructor(simProps: ViscousSimProps) {
        ...
        (this.props as Record<string, unknown>).output0 = simProps.dst_;
        (this.props as Record<string, unknown>).output1 = simProps.dst;
        this.init();
      }

      update(opts: { viscous: number; iterations: number; dt: number }) {
        const output0 = (this.props as Record<string, THREE.WebGLRenderTarget>).output0;
        const output1 = (this.props as Record<string, THREE.WebGLRenderTarget>).output1;
        let fbo_out: THREE.WebGLRenderTarget | undefined;
        this.uniforms!.v.value = opts.viscous;
        for (let i = 0; i < opts.iterations; i++) {
          if (i % 2 === 0) {
            this.uniforms!.velocity_new.value = output0.texture;
            this.props.output = output1;
          } else {
            this.uniforms!.velocity_new.value = output1.texture;
            this.props.output = output0;
          }
          this.uniforms!.dt.value = opts.dt;
          super.update();
          fbo_out = this.props.output!;
        }
        return fbo_out!;
      }
    }

    type DivergenceSimProps = {
      cellScale: THREE.Vector2;
      boundarySpace: THREE.Vector2;
      src: THREE.WebGLRenderTarget;
      dst: THREE.WebGLRenderTarget;
    ...
    class Poisson extends ShaderPass {
      ...
      update(opts: { iterations: number }) {
        ...
        for (let i = 0; i < opts.iterations; i++) {
          if (i % 2 === 0) {
            this.uniforms!.pressure.value = output0.texture;
            this.props.output = output1;
          } else {
            this.uniforms!.pressure.value = output1.texture;
            this.props.output = output0;
          }
          super.update();
        }
    ...
    const applyOptionsFromProps = () => {
      const webglCurrent = webglRef.current;
      if (!webglCurrent) return;
      const sim = webglCurrent.output?.simulation;
      if (!sim) return;
    ...
    const ro = new ResizeObserver(() => {
      ...
      resizeRafRef.current = requestAnimationFrame(() => {
        const webglCurrent2 = webglRef.current;
        if (!webglCurrent2) return;
        webglCurrent2.resize();
      });
    });
